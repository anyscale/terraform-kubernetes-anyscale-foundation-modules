...
- Do not interrupt (`Ctrl-C`, `kill`) a `deploy` or `teardown` run partway through. Both call Terraform, and stopping the wrapper script does not stop an in-flight `terraform apply`/`destroy` it already started — the orphaned process keeps holding the local state lock (`infra/terraform/.terraform.tfstate.lock.info`) until it finishes on its own. `setup_run_init` also takes its own harness-level run lock (`.cache/aks-anyscale-sample-harness/run.lock`) for the whole `deploy`/`verify`/`proof`/`teardown` invocation, not just the moment a `terraform` command is executing, so a second invocation started while one is already running fails immediately with a clear message instead of interleaving with it. (An earlier version of this sample relied on Terraform's own state lock alone; that lock is retried-through as transient contention rather than treated as fatal, so two long-running `teardown` invocations left running concurrently — e.g. one kicked off days earlier and forgotten about — could genuinely race each other's Azure API calls for the entire run.) If you interrupted a run, let the orphaned `terraform` process finish (`ps aux | grep terraform`) or wait out `run.lock`, rather than deleting either lock file by hand.
...
- If Private Link to the Anyscale control plane is enabled and was approved, `teardown`'s destroy of `module.anyscale_privatelink.azurerm_private_endpoint.this` can hang for up to an hour before failing with a client-side `polling after Delete: context deadline exceeded`, then keep failing on every retry. The underlying cause is a resource lock Anyscale holds on the private-endpoint-connection-proxy object backing the connection, on their side of the cross-tenant Private Link Service — confirmed directly with `az network private-endpoint delete`, which fails immediately with `ScopeLocked` against a resource ID under Anyscale's resource group. This is not fixable from this side; ask your Anyscale contact to release the lock (or disconnect the connection) before the private endpoint, its subnet, and the VNet can be deleted. Everything else in the resource group tears down normally.
