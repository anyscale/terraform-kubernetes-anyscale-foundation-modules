...
> **Give the operator a couple of minutes after `apply` returns.** The extension resource completes once the operator Deployment exists, not once the operator has registered the cluster's instance types with the control plane. A job submitted in that window fails immediately with `API Exception (400) from GET /cluster_computes/default ... "No CPU instance types found for this cloud."` Nothing is wrong: retry. In a from-empty run the error cleared about 1m45s after apply finished (about 2.5 min after the operator pod started).
