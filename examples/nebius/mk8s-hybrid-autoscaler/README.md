...
* [Anyscale CLI](https://docs.anyscale.com/cli/) (`anyscale`)
...
anyscale:
  ...
  cli_token: "aph0_xxxxx"               # Optional: set directly or rely on `anyscale login`
...
1. **Authenticate the Anyscale CLI:**
   - Recommended: run `anyscale login` (writes the token to `~/.anyscale/credentials.json`)
   - Alternatively, create a token via https://console.anyscale.com → Settings → API Keys and paste it into `config.yaml` under `anyscale.cli_token`
...
- Prompt you to paste the Cloud Deployment ID (if not already captured via CLI)

After the cloud is registered, rerun `./deploy/install-operator.sh` — the script automatically discovers the deployment ID via the Anyscale CLI (or you can override it by exporting `ANYSCALE_CLOUD_DEPLOYMENT_ID`).

### Step 5: Deploy Kubernetes Cluster

Deploy the Nebius MK8s infrastructure (cluster + node groups + networking):
...
- IAM service accounts for node groups
...
With the cluster running, deploy the operator separately via Helm (keeps infrastructure and application lifecycles decoupled). Make sure `nebius`, `terraform`, `helm`, `kubectl`, and `jq` are on your `PATH`:

```bash
cd examples/nebius/mk8s-hybrid-autoscaler
./deploy/install-operator.sh
```

The script will:
- Read cluster + network settings from `config.yaml`, discover the CLI token (from `config.yaml` or `~/.anyscale/credentials.json`), and look up the cloud deployment ID automatically
- Create/rotate Nebius access keys for object storage
- Fetch kubeconfig for the MK8s cluster
- Install/upgrade the vendored Anyscale operator chart with `values/anyscale-operator.yaml`

Re-run the script any time you need to upgrade the operator or rotate credentials.
Access keys generated by the script expire after 7 days by default; adjust or delete them sooner if required by your security policies.
...
### Step 9: Complete Anyscale Registration
...
```bash
# (Optional) Uninstall operator before tearing down the cluster
helm uninstall anyscale-operator -n anyscale-system || true
...
Don't forget to clean up Nebius IAM access keys that were generated for the operator:

```bash
nebius iam access-key list --parent-id <project-id>
nebius iam access-key delete --id <access-key-id>
```
